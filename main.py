...
def show_commands(message):
    ...
    if len(command_parts) != 1 and len(command_parts) != 2:
        ...
        bot.send_message(message.chat.id, commands.get(message.text.split(" ")[1], "Comando desconocido"))
...
def new_roadmap(message):
    ...
    if len(command_parts) != 1:
        ...
        try:
            ...
            bot.reply_to(message, "Error al crear archivo de puntos a hablar")
        bot.send_message(message.chat.id, "Archivo creado")   
...
def add_point_of_day(message):
    ...
    if len(command_parts) < 2:
            ...
            with open('points_of_day.txt', 'r') as points_of_day_read:
                for point_of_day in points_of_day_read:
                    if (point_of_day.strip() == "PDD: " + new_point_of_day):
                        bot.reply_to(message, "Punto del día añadido anteriormente")
                        return
        except IOError:
            bot.reply_to(message, "No existe ningún archivo de puntos del día, puede crearlo con /new")
        ...
        with open('points_of_day.txt', 'a') as points_of_day_write:
            ...
            points_of_day_write.write("\n")
        bot.send_message(message.chat.id,  new_point_of_day + " añadido a los puntos del día")
...
def list_points_of_day(message):
    ...
    if len(command_parts) != 1 and len(command_parts) != 2:
        ...
        try: 
            ...
            bot.send_message(message.chat.id, list_points_of_day)
        except IOError:
            bot.reply_to(message, "No existe ningún archivo de puntos del día, puede crearlo con /new")
    else:
        try:
            ...
            with open('points_of_day.txt', 'r') as points_of_day_read:
                for point_of_day in points_of_day_read:
                    if point_of_day.split("-")[0] == "PDD: " + command:
                        bot.send_message(message.chat.id, point_of_day.split("-")[0] + ": " + point_of_day.split("-")[1])
        except IOError:
            bot.reply_to(message, "No existe ningún archivo de puntos del día, puede crearlo con /new")
...
def remove_point_of_day(message):
    ...
    if len(command_parts) != 2:
        ...
        try: 
            ...
            bot.reply_to(message, "No existe ningún archivo de puntos del día") 

@bot.message_handler(commands=['delete'])
def delete(message):
    bot.reply_to(message.text, "Comando no implementado, en proceso...")
